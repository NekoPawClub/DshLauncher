...
- exe 版本号格式 `YY.MM.DD.NN` (年.月.日.当日第几次发布)：CI 发布时经环境变量 `DSHLAUNCHER_VERSION` 传入 build.rs 嵌入；本地构建自动取构建当天日期 + 0
...
- dsh 输出日志：`npx -y @deepseek-ai/dsh web` 的 stdout/stderr 追加写入 `%USERPROFILE%/.dsh/dsh.log` (启动慢/失败时查看此文件定位原因；超过 1 MiB 自动清空)
